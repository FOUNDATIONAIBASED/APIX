...
//                            MessagingNotification.updateDownloadFailedNotification(this);
    ...
    /** Removed from public SDK stubs on API 31+; use reflection for older devices. */
    private static int startUsingNetworkFeatureCompat(ConnectivityManager cm, int networkType, String feature) {
        try {
            Method m = ConnectivityManager.class.getMethod("startUsingNetworkFeature", int.class, String.class);
            return (Integer) m.invoke(cm, networkType, feature);
        } catch (Throwable t) {
            Timber.w(t, "startUsingNetworkFeature not available");
            return 0;
        }
    }

    private static void stopUsingNetworkFeatureCompat(ConnectivityManager cm, int networkType, String feature) {
        try {
            Method m = ConnectivityManager.class.getMethod("stopUsingNetworkFeature", int.class, String.class);
            m.invoke(cm, networkType, feature);
        } catch (Throwable t) {
            Timber.w(t, "stopUsingNetworkFeature not available");
        }
    }
    ...
    protected int beginMmsConnectivity() throws IOException {
        ...
        int result = startUsingNetworkFeatureCompat(
